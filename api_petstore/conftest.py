...
def store_post_order_without_body():
    resp = requests.post(url='https://petstore.swagger.io/v2/store/order')
    yield resp
...
@pytest.fixture
def store_post_order_with_none_id():
    resp = requests.post(
        url='https://petstore.swagger.io/v2/store/order',
        json=content.preset_store_order_2)
    yield resp


@pytest.fixture
def store_post_order_with_text_id():
    resp = requests.post(
        url='https://petstore.swagger.io/v2/store/order',
        json=content.preset_store_order_3)
    yield resp
